...
| ★ [The hardlink hazard](the-hardlink-hazard.md) | `node_modules` is an immutable install and a mutable scratch dir in one folder, so every way of copying it breaks something: hardlinks let one worktree's test run turn another's suite red, plain `cp -r` carries a stale build record that makes `tsc` exit 0 emitting nothing, symlinks share the live thing. Reproduced with controls ([repro](examples/hardlink-repro/), [plain-English walkthrough](hardlink-hazard-explained.md)); a correctness bug the guides all describe as an optimization. |
| [Two worktrees, one node_modules](two-worktrees-one-node-modules.md) | The public write-up: two paste-able demos, no repo to clone. A passing suite reported as failed because of a run in another directory, and a green pipeline that compiled nothing and tested nothing. Applies to CI `node_modules` caching too, not just agents. |
