...
| [Settled is a human word](settled-is-a-human-word.md) | The model promotes its own proposals to settled status — an ADR stamped "accepted" that nobody ruled, fixes "queued" in no queue, a tracker filing unverified findings as to-dos — and it keeps doing it after accurately admitting it, five catches in one day. Status words are human acts; a decision log is the only mint; everything else is proposed. |
