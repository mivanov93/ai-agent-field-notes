export const meta = {
  ...
  description: 'Deep research harness (cheaper variant) — same fan-out/verify/synthesize as deep-research, but the fan-out runs on Haiku (search) and Sonnet (fetch + verify); only scope + synthesize inherit the session model.',
...
// Question is passed via Workflow({scriptPath: '<this file>', args: '<question>'}).
...
// Model assignment — keep Opus (the inherited session model) only where report
// quality actually lives: scope (1 call) and synthesize (1 call). Push the whole
// fan-out onto cheaper models. Verify is ~75 calls and is the cost center.
const MODEL_SEARCH = "haiku"   //  5 calls — just rank WebSearch hits
const MODEL_FETCH = "sonnet"   // ≤15 calls — WebFetch + extract falsifiable claims
const MODEL_VERIFY = "sonnet"  // ≤75 calls — adversarial verdict, the cost center

// ─── Schemas ───
const SCOPE_SCHEMA = {
  // Only `angles` is consumed downstream (pipeline + logs); requiring `question`
  // and `summary` — which nothing reads — failed whole runs when the model bled
  // those fields into `question` under a long prompt. Require only what we use.
...
if (!QUESTION) {
  return { error: "No research question provided. Pass it as args: Workflow({scriptPath: '<this file>', args: '<question>'})." }
}
...
const voted = (await parallel(
  rankedClaims.map(claim => () =>
    parallel(
      ...
      // Three outcomes (anthropics/claude-code#69883 — infra failure must not read as "refuted"):
